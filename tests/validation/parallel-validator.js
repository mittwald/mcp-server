 ...
 */
async function executeCli(command, args) {
 ...
 */
function deepCompare(obj1, obj2, path = '', ignoreFields = []) {
    const discrepancies = [];
    // Skip ignored fields
    ...
    if (Array.isArray(obj1) && Array.isArray(obj2)) {
        ...
        for (let i = 0; i < maxLen; i++) {
            const itemDiscrepancies = deepCompare(obj1[i], obj2[i], `${path}[${i}]`, ignoreFields);
    ...
    for (const key of allKeys) {
        ...
        const itemDiscrepancies = deepCompare(obj1[key], obj2[key], newPath, ignoreFields);
        discrepancies.push(...itemDiscrepancies);
    }
    return discrepancies;
}
 ...
 */
export async function validateToolParity(options) {
    // Execute CLI
...
//# sourceMappingURL=parallel-validator.js.map