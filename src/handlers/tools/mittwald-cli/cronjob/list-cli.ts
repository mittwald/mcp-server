...
export const handleCronjobListCli: MittwaldToolHandler<MittwaldCronjobListCliArgs> = async (args, context) => {
  try {
    ...
    if (result.exitCode !== 0) {
      ...
      return formatToolResponse(
        "error",
        `Failed to list cronjobs: ${errorMessage}`
      );
    }
