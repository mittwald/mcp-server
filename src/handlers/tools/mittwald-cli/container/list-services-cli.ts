...
export const handleContainerListCli: MittwaldToolHandler<MittwaldContainerListArgs> = async (args) => {
  try {
    // Build CLI command arguments
    const cliArgs: string[] = ['container', 'list'];
    ...
    if (result.exitCode !== 0) {
      ...
      return formatToolResponse(
        "error",
        `Failed to list containers: ${errorMessage}`
      );
    }
