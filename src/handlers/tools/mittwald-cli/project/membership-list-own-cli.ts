...
export const handleProjectMembershipListOwnCli: MittwaldToolHandler<MittwaldProjectMembershipListOwnArgs> = async (args, context) => {
  try {
    // Build CLI command arguments
    const cliArgs: string[] = ['project', 'membership', 'list-own'];
    ...
    if (result.exitCode !== 0) {
      ...
      return formatToolResponse(
        "error",
        `Failed to list own project memberships: ${errorMessage}`
      );
    }
    ...
    try {
      ...
      if (data.length === 0) {
        return formatToolResponse(
          "success",
          "No project memberships found for the current user",
          []
        );
      }
      ...
      return formatToolResponse(
        "success",
        `Found ${data.length} project membership(s) for the current user`,
        formattedData
      );
