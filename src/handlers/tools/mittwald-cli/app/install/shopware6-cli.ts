...
export const handleAppInstallShopware6Cli: MittwaldCliToolHandler<MittwaldAppInstallShopware6Args> = async (args) => {
  try {
    ...
    if (result.exitCode !== 0) {
      ...
      if (errorMessage.includes('403') || errorMessage.includes('Forbidden') || errorMessage.includes('Permission denied')) {
        return formatToolResponse(
          "error",
          `Permission denied when installing Shopware 6. Complete OAuth sign-in and ensure the Mittwald CLI is authenticated.\nError: ${errorMessage}`
        );
      }
    ...
    if (!appInstallationId) {
      ...
      return formatToolResponse(
        "success",
        args.quiet ? result.stdout : `Shopware 6 installation started successfully`,
        {
