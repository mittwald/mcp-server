...
interface MittwaldAppInstallContaoArgs {
  projectId: string;
  version?: string;
  host?: string;
  adminUser?: string;
...
function buildCliArgs(args: MittwaldAppInstallContaoArgs): string[] {
...
function mapCliError(error: CliToolError, args: MittwaldAppInstallContaoArgs): string {
  ...
  if (stderr.includes('permission') || stderr.includes('forbidden')) {
    return `Permission denied when installing Contao. Complete OAuth sign-in and ensure the Mittwald CLI is authenticated.\nError: ${error.stderr || error.message}`;
  }

  if (stderr.includes('invalid') && stderr.includes('version')) {
    return `Invalid Contao version: ${args.version ?? 'latest'}. Please use a valid version number.\nError: ${error.stderr || error.message}`;
  }

  if (stderr.includes('already exists') || stderr.includes('conflict')) {
    return `Contao installation already exists or conflicts with an existing installation.\nError: ${error.stderr || error.message}`;
  }
...
export const handleAppInstallContaoCli: MittwaldCliToolHandler<MittwaldAppInstallContaoArgs> = async (args) => {
  ...
  try {
    const result = await invokeCliTool({
      toolName: 'mittwald_app_install_contao',
      argv,
    ...
    if (!installationId) {
      return formatToolResponse(
        'success',
        args.quiet ? output : 'Contao installation started successfully',
        {
    ...
    const successMessage = args.quiet
      ? installationId
      : args.wait
        ? `Contao installation completed successfully with ID ${installationId}`
        : `Contao installation started with ID ${installationId}`;
