...
export const handleAppInstallContaoCli: MittwaldCliToolHandler<MittwaldAppInstallContaoArgs> = async (args) => {
  try {
    ...
    if (result.exitCode !== 0) {
      ...
      if (errorMessage.includes('403') || errorMessage.includes('Forbidden') || errorMessage.includes('Permission denied')) {
        return formatToolResponse(
          "error",
          `Permission denied when installing Contao. Complete OAuth sign-in and ensure the Mittwald CLI is authenticated.\nError: ${errorMessage}`
        );
      }
      ...
      if (errorMessage.includes('Invalid') && errorMessage.includes('version')) {
        return formatToolResponse(
          "error",
          `Invalid Contao version: ${args.version}. Please use a valid version number.\nError: ${errorMessage}`
        );
      }
      ...
      if (errorMessage.includes('already exists') || errorMessage.includes('conflict')) {
        return formatToolResponse(
          "error",
          `Contao installation already exists or conflicts with existing installation.\nError: ${errorMessage}`
        );
      }
      
      return formatToolResponse(
        "error",
        `Failed to install Contao: ${errorMessage}`
      );
    }
    ...
    if (args.quiet) {
      ...
      // Example: "Contao installation started with ID app-xxxxx"
    ...
    if (!appInstallationId) {
      ...
      return formatToolResponse(
        "success",
        args.quiet ? result.stdout : `Contao installation started successfully`,
        {
    ...
    const successMessage = args.quiet ? 
      appInstallationId :
      args.wait ? 
        `Contao installation completed successfully with ID ${appInstallationId}` :
        `Contao installation started with ID ${appInstallationId}`;
