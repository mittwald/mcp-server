...
export const handleAppInstallShopware6: MittwaldToolHandler<MittwaldAppInstallShopware6Args> = async (args, { mittwaldClient }) => {
  try {
    ...
    if (version === "latest") {
      ...
      if (!appVersion) {
        throw new Error(`no version ${version} found for app Shopware 6`);
      }
    }
    ...
    const successText = args.wait
      ? "Your Shopware 6 installation is now complete. Have fun! 🎉"
      : "Your Shopware 6 installation has started. Have fun when it's ready! 🎉";
