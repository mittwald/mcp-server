...
export const handleAppInstallNextcloud: MittwaldToolHandler<MittwaldAppInstallNextcloudArgs> = async (args, { mittwaldClient }) => {
  try {
    ...
    if (version === "latest") {
      ...
      if (!appVersion) {
        throw new Error(`no version ${version} found for app Nextcloud`);
      }
    }
    ...
    const successText = args.wait
      ? "Your Nextcloud installation is now complete. Have fun! 🎉"
      : "Your Nextcloud installation has started. Have fun when it's ready! 🎉";
