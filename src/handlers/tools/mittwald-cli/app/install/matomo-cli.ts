...
function buildCliArgs(args: MittwaldAppInstallMatomoArgs): string[] {
...
function mapCliError(error: CliToolError, args: MittwaldAppInstallMatomoArgs): string {
  ...
  if (stderr.includes('permission') || stderr.includes('forbidden')) {
    return `Permission denied when installing Matomo. Complete OAuth sign-in and ensure the Mittwald CLI is authenticated.\nError: ${error.stderr || error.message}`;
  }

  if (stderr.includes('invalid') && stderr.includes('version')) {
    return `Invalid Matomo version: ${args.version ?? 'latest'}. Please use a valid version number.\nError: ${error.stderr || error.message}`;
  }

  if (stderr.includes('already exists') || stderr.includes('conflict')) {
    return `Matomo installation already exists or conflicts with an existing installation.\nError: ${error.stderr || error.message}`;
  }
...
export const handleAppInstallMatomoCli: MittwaldCliToolHandler<MittwaldAppInstallMatomoArgs> = async (args) => {
  ...
  try {
    const result = await invokeCliTool({
      toolName: 'mittwald_app_install_matomo',
      argv,
