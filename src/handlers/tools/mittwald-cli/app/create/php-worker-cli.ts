...
export const handleAppCreatePhpWorkerCli: MittwaldToolHandler<MittwaldAppCreatePhpWorkerArgs> = async (args, context) => {
  try {
    ...
    if (result.exitCode !== 0) {
      ...
      if (errorMessage.includes('403') || errorMessage.includes('Forbidden') || errorMessage.includes('Permission denied')) {
        return formatToolResponse(
          "error",
          `Permission denied when creating PHP worker app. Check if your API token has app management permissions.\nError: ${errorMessage}`
        );
      }
      ...
      return formatToolResponse(
        "error",
        `Failed to create PHP worker app: ${errorMessage}`
      );
    }
