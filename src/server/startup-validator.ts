/**
 * Startup Security Validator for Main MCP Server
 *
...
/**
 * Validates CORS_ORIGIN is properly configured in production.
 *
 * @throws StartupValidationError if CORS_ORIGIN is '*' or unset in production
 */
function validateCorsOrigin(): void {
  ...
  if (!corsOrigin) {
    throw new StartupValidationError(
      'CORS_ORIGIN must be set in production - specify allowed origin(s) like "https://example.com"'
    );
  }
