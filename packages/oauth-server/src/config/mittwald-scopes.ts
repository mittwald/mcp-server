...
const defaultScopeSet = new Set(scopeConfiguration.defaultScopes);
...
function loadScopeConfiguration(): MittwaldScopeConfiguration {
  ...
  for (const scope of upstream) {
    if (!supported.includes(scope)) {
      logger.warn('Mittwald scope configuration: upstream scope missing from supportedScopes list', { scope });
    }
  }

  logger.info('Loaded Mittwald scope configuration', {
    configPath,
