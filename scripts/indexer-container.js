...
// Configuration - using internal service name
const OPENSEARCH_URL = 'http://opensearch:9200';
...
async function main() {
  ...
  while (retries > 0) {
    try {
      await axios.get(OPENSEARCH_URL);
  ...
  if (retries === 0) {
    console.error('OpenSearch is not ready after 60 seconds');
    process.exit(1);
  }
