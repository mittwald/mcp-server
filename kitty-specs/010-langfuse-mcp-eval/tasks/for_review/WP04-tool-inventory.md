...
- [x] Reclassify tools to match the domain breakdown specified in the prompt (e.g., move `login/*` to `misc`, adjust other mis-bucketed tools) and regenerate `evals/inventory/tools.json`.
  - **DONE**: Moved `login/reset`, `login/status`, `login/token` from identity to misc
- [x] Verify the `domains` map totals exactly match the expected per-domain counts from the prompt and update the file accordingly.
  - **DONE**: Verified, but found spec inconsistencies. Added `spec_reconciliation` section to tools.json documenting:
    - 3 phantom tools in spec that don't exist: `context/get-session`, `context/set-session`, `context/reset-session`
    - 1 naming difference: spec says `container/list` but actual is `container/list-services`
    - 1 missing from spec: `database/index` exists in codebase but not spec
    - domains-mail spec header says 20 but enumeration shows 21
