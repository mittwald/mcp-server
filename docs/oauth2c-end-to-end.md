...
- The OAuth proxy deployed at `https://mittwald-oauth-server.fly.dev`.
...
curl -X POST https://mittwald-oauth-server.fly.dev/reg \
...
The Mittwald OAuth proxy at `https://mittwald-oauth-server.fly.dev` is working correctly:
