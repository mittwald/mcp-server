...
| login status | Checks your current authentication status | ⚠️ Missing |  |  | Allowed missing (intentional): MCP server uses per-command token injection; CLI login state not applicable — Allowed missing (intentional): MCP server uses per-command token injection; CLI login state not applicable — Allowed missing (intentional): MCP server uses per-command token injection; CLI login state not applicable — Allowed missing (intentional): MCP server uses per-command token injection; CLI login state not applicable — Allowed missing (intentional): MCP server uses per-command token injection; CLI login state not applicable — Allowed missing (intentional): MCP server uses per-command token injection; CLI login state not applicable — Allowed missing (intentional): MCP server uses per-command token injection; CLI login state not applicable |
