...
| app dependency list | Get all available dependencies | ⚠️ Missing |  |  | Allowed missing (intentional): Workstream C1 will implement dependency inspection wrappers; CLI command temporarily excluded. |
| app dependency update |  | ⚠️ Missing |  |  | Allowed missing (intentional): Workstream C1 backlog item; update functionality to be wrapped once CLI parser is stabilized. |
| app dependency versions | Get all available versions of a particular dependency | ⚠️ Missing |  |  | Allowed missing (intentional): Workstream C1 backlog item; awaiting shared schema for dependency metadata. |
...
| container update | Updates attributes of an existing container such as image, environment variables, etc. | ⚠️ Missing |  |  | Allowed missing (intentional): Workstream C2: non-interactive container lifecycle command pending wrapper. |
...
| database mysql user create | Create a new MySQL user | ⚠️ Missing |  |  | Allowed missing (intentional): Workstream C3: MySQL user management tooling in progress. |
| database mysql user delete | Delete a MySQL user | ⚠️ Missing |  |  | Allowed missing (intentional): Workstream C3: MySQL user management tooling in progress. |
| database mysql user get | Get a MySQL user. | ⚠️ Missing |  |  | Allowed missing (intentional): Workstream C3: MySQL user management tooling in progress. |
| database mysql user list | List MySQL users belonging to a database. | ⚠️ Missing |  |  | Allowed missing (intentional): Workstream C3: MySQL user management tooling in progress. |
| database mysql user update | Update an existing MySQL user | ⚠️ Missing |  |  | Allowed missing (intentional): Workstream C3: MySQL user management tooling in progress. |
...
| database redis create |  | ⚠️ Missing |  |  | Allowed missing (intentional): Workstream C3: Redis lifecycle tooling pending CLI schema finalisation. |
| database redis get | Get a Redis database. | ⚠️ Missing |  |  | Allowed missing (intentional): Workstream C3: Redis lifecycle tooling pending CLI schema finalisation. |
| database redis list | List Redis databases belonging to a project. | ⚠️ Missing |  |  | Allowed missing (intentional): Workstream C3: Redis lifecycle tooling pending CLI schema finalisation. |
...
| database redis versions | List available Redis versions. | ⚠️ Missing |  |  | Allowed missing (intentional): Workstream C3: Redis lifecycle tooling pending CLI schema finalisation. |
...
| CLI Command | Description | Status | MCP Tool | Tool Definition | Notes |
| --- | --- | --- | --- | --- | --- |
| org delete | Delete an organization | ⚠️ Missing |  |  | Allowed missing (intentional): Workstream C4: Organisation CRUD wrappers planned following permission review. |
| org get | Get an organization profile. | ⚠️ Missing |  |  | Allowed missing (intentional): Workstream C4: Organisation CRUD wrappers planned following permission review. |
| org invite | Invite a user to an organization. | ⚠️ Missing |  |  | Allowed missing (intentional): Workstream C4: Invitation flow being consolidated before exposing via MCP. |
...
| org list | Get all organizations the authenticated user has access to. | ⚠️ Missing |  |  | Allowed missing (intentional): Workstream C4: Organisation listing requires pagination harmonisation. |
| org membership list | List all memberships belonging to an organization. | ⚠️ Missing |  |  | Allowed missing (intentional): Workstream C4: Membership endpoints pending shared schema. |
| org membership list-own | List all organization memberships for the executing user. | ⚠️ Missing |  |  | Allowed missing (intentional): Workstream C4: Membership endpoints pending shared schema. |
| org membership revoke | Revoke a user's membership to an organization | ⚠️ Missing |  |  | Allowed missing (intentional): Workstream C4: Membership revoke requires audit logging design. |
...
| CLI Command | Description | Status | MCP Tool | Tool Definition | Notes |
| --- | --- | --- | --- | --- | --- |
| volume create | Creates a new named volume in the project stack. The volume will be available for use by containers. | ⚠️ Missing |  |  | Allowed missing (intentional): Workstream C5: Volume management wrappers awaiting context scaffolding. |
| volume delete | Removes named volumes from the project stack. Be careful as this will permanently delete the volume data. | ⚠️ Missing |  |  | Allowed missing (intentional): Workstream C5: Volume management wrappers awaiting context scaffolding. |
| volume list | List volumes belonging to a project. | ⚠️ Missing |  |  | Allowed missing (intentional): Workstream C5: Volume management wrappers awaiting context scaffolding. |
