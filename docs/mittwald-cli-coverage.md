...
| login status | Checks your current authentication status | ⚠️ Missing |  |  | Intentionally disabled; MCP server relies on token-based auth and per-command --token injection. — Allowed missing (intentional): MCP server uses per-command token injection; CLI login state not applicable |
