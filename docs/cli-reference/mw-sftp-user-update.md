...
```
$ mw sftp-user update --help
Update an existing SFTP user

USAGE
  $ mw sftp-user update SFTP-USER-ID [-q] [--expires <value>] [--description
    <value>] [--public-key <value> | --password <value>] [--access-level
    read|full] [--directories <value>...] [--enable | --disable]
...
FLAGS
  -q, --quiet                   suppress process output and only display a
      ...
      --directories=<value>...  Specify directories to restrict this SFTP users
                                access to.
...
FLAG DESCRIPTIONS
  ...
  --disable  Disable the SFTP user.
    ...
    Set the status of the SFTP user to inactive. Access by this user will be
    disable.

  --enable  Enable the SFTP user.

    Set the status of the SFTP user to active. Access by this user will be
    enabled.
